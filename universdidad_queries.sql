...
# 8.Retorna un llistat amb el nom de tots els departaments que tenen professors/es que imparteixen alguna assignatura en el Grau en Enginyeria Informàtica (Pla 2015).
    ...
    SELECT DISTINCT(d.nombre) FROM grado g
             INNER JOIN asignatura a
             ON g.id = a.id_grado
...
# 9.Retorna un llistat amb tots els alumnes que s'han matriculat en alguna assignatura durant el curs escolar 2018/2019.
    ...
    SELECT p.apellido1, p.apellido2, p.nombre FROM curso_escolar ce
             INNER JOIN alumno_se_matricula_asignatura asma
...
# l'any d'inici del curs escolar i una altra amb el nombre d'alumnes matriculats.
    ...
    SELECT ce.anyo_inicio, COUNT(asma.id_alumno) FROM curso_escolar ce
    INNER JOIN alumno_se_matricula_asignatura asma
    ON ce.id = asma.id_curso_escolar
...
# 11.Retorna un llistat amb els professors/es que tenen un departament associat i que no imparteixen cap assignatura.
    ...
    SELECT p.*, pr.id_departamento, pr.id_profesor, a.id_profesor FROM persona p
